...
- 🇺🇦 Виправлено помилку інтерактивного підсвічування термінів (при виборі терміна в елементі `#term-tree`).

## v2.0.0-beta-70, 2020-02-27

### 🔴 Виправлення помилок

- 🇺🇦 Протестовано та виправлено роботу бібліотеки Konspekt.exe (для української мови версії від 03-11-2018) з файлами великого розміру.
- 🇺🇦 Вимкнено режим налагодження `wine` (додано змінну середовища `export WINEDEBUG=-all`).
- 🇺🇦 Виправлено помилку інтерактивного підсвічування термінів (якщо частота появи терміна в тексті перевищує 300 разів, в цьому випадку буде відключене інтерактивне підсвічування терміна задля коректного роботи інтерфейсу веб-застосунку).

## v2.0.0-beta-67, 2020-02-24

### 👍 Покращення

- 🇺🇦 🇬🇧 На головну веб-сторінку додатка додано номер бета-версії.

## v2.0.0-beta-66, 2020-02-22

### 🔴 Виправлення помилок

- 🇺🇦 Виправлено список змін.

## v2.0.0-beta-65, 2020-02-22

### ⚠️ Зауваження

- 🇺🇦 Додано обмеження на розмір оброблюваного файлу, яке становить 2 мб.
- 🇺🇦 Протестовано роботу бібліотеки Konspekt.exe (для української мови версії від 03-11-2018) з файлами великого розміру.
- 🇺🇦 Змінено специфікацію `JSON`-файлу проекту (Konspekt.exe для української мови версії від 03-11-2018).
...
- 🇺🇦 🇬🇧 Додана стартова сторінка вибору мови Конспекту (для обробки документів української або англійською).
- 🇺🇦 Зменшено час аналізу файлів бібліотекою Konspekt.exe (для української мови версії від 03-11-2018).
- 🇺🇦 При виборі терміна в елементах `<select>` `#uploadResultList` (спиcок термінів) та `<select>` `#term-tree` (дерево термінів), якщо обчислення займає тривалий час, то інтерфейс відображає процес завантаження.

### 🔴 Виправлення помилок

- 🇺🇦 Виправлені функції сортування в елементі `<select>` `#uploadResultList`.
- 🇺🇦 Виправлені технологія та функції підсвічування термінів (`function mark(text)`, `function markTerms(term)`) в елементі `#text-content`.
...
- 🇺🇦 🇬🇧 Видалено невикористовувані `JavaScript`-бібліотеки, зокрема, `tippy.js`.

## v2.0.0-beta-41, 2019-12-25

### 🔴 Виправлення помилок

- 🇺🇦 Виправлено проблему з попередньо визначеними об'єктами XML `r'&|>|<|_|"|\.\.+|\s\s+'` (Konspekt.exe для української мови версії від 03-11-2018).
- 🇺🇦 Виправлені проблеми з кодиваннями utf-8/windows-1251.

### 👍 Покращення

- 🇺🇦 Додана функція динамічного визначення часу на тривалість роботи Konspekt.exe для української мови версії від 03-11-2018:
  ```python
...
- 🇺🇦 Виправлено помилки при decode()/encode().

## v2.0.0-beta-12, 2019-12-20

### 👍 Покращення

- 🇺🇦 Додана детекція кодування бібліотекою chardet.
...
- 🇺🇦 Виправлено помилки при кодуванні: decode the file as CP1251 ignoring any errors.

## v2.0.0-beta-5, 2019-12-19

### 👍 Покращення

- 🇺🇦 Оновлено Konspekt.exe для української мови до версії від 03-11-2018.
- 🇺🇦 Оновлено документацію та список змін.

## v2.0.0-beta-4, 2019-12-18

### 🔴 Виправлення помилок

- 🇬🇧 Виправлені помилки англійської локалізації інтерфейсу користувача англомовної частини.
...
- 🇬🇧 Додана англійська локалізація інтерфейсу користувача англомовної частини.
...
### 🌟 Initial commit for multilingual version 🇬🇧, 🇺🇦
