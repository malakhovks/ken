...
//SAVE_TABLE_TO_XLSX_____________________________________________________________________________________________________
...
//OPEN_XLSX_TO_TABLE______________________________________________________________________________________________________
$('#button-open-xlsx-document').click(function () {
    // alert("🚨 Please upload files that were generated by this application only!");
    let canOpen = confirm("🚨 Please upload files that were generated by this application only!");
    if (canOpen) {
            ...
            if (typeof (reader) != "undefined") {
                ...
                if (reader.readAsBinaryString) {
                    reader.onload = function (e) {
                        ...
                        var result = XLSX.utils.sheet_to_json(firstSheet, { raw: false, header: 1, defVal: "" });

                        console.log('Max row index: ' + result.reduce(function (maxI, el, i, arr) { return el.length > arr[maxI].length ? i : maxI; }, 0));
                        console.log('Row quantity of the cells: ' + result[result.reduce((p, c, i, a) => a[p].length > c.length ? p : i, 0)].length);

                        result = result.filter(e => e.length)
                        for (res of result) {
                            res.unshift(null);
                        }

                        result.unshift(Array(result[result.reduce((p, c, i, a) => a[p].length > c.length ? p : i, 0)].length).fill(null));
...
//OPEN_XLSX_TO_TABLE_____________________________________________________________________________________________________
